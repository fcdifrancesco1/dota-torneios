...
function renderLiveMatchDetail(g, streamLinks) {
  ...
  const rPlayers = getSidePlayers(sb, "radiant");
  const dPlayers = getSidePlayers(sb, "dire");
  const netWorthList = (players) => players.slice().sort((a, b) => (a.player_slot ?? 0) - (b.player_slot ?? 0)).map((p) =>
    `<div class="nw-row"><img src="${heroImg(p.hero_id)}" alt="">${(p.net_worth || 0).toLocaleString("pt-BR")}</div>`
  ).join("");
